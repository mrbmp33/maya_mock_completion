...
from mmc_output.mmc_node_type_ids import _TYPE_STR_TO_ID, _TYPE_INT_TO_STR, _API_TYPES_INT_TO_FN_TYPE_STR
...
def _create_node_from_type(
    ...
    try:
        if isinstance(type_id, str):
            type_alias = type_id
            type_str = mmc_node_types_alias_map.NODE_TYPES_ALIAS_MAP.get(type_id, type_id)  # Resolve type alias
            # noinspection PyProtectedMember
            obj_id = MTypeId(_TYPE_STR_TO_ID[type_str])
        else:
            obj_id = type_id
    ...
    if shape_type := node_types_to_shapes.NODE_TYPES_TO_SHAPES.get(type_str):
        ...
        if parent is None:
            parent_mobject = _create_node_from_type(MTypeId(_TYPE_STR_TO_ID['kTransform']), name=parent_name)
        else:
    ...
    mobject._name = hierarchy.find_first_available_name(name or f'{type_alias}1', parent_name=parent_name)

    # Flag as alive
    mobject._is_null = False

    # (If applicable) Initialize type-specific fields
    if type_str == 'kNurbsCurve' or type_str == 'kBezierCurve':
...
def _attr_from_name_and_node(attr_name: str, owner_node: "MObject") -> Tuple['MObject', Optional[dict]]:
    ...
    node_type = owner_node.apiTypeStr
...
def _get_node_type(mobject: 'MObject') -> str:
    try:
        ...
        node_type = mmc_node_types_alias_map.NODE_TYPES_ALIAS_MAP.get(node_type, node_type)
...
def _get_attribute_properties(node_type, attr_name) -> Tuple[dict, str, str]:
    """Gets an attribute dictionary, long name and short name from the attribute_properties module
    based on the <MFn key (starts with k)> and <attr_name>."""
...
class MFn(object):
    ...
    kInvalid = 0
    kInverseLinearInterpolation = 1173
    ...
    kMultiply = 1181
    kMultiplyDivide = 448
    kMultiplyPointByMatrix = 1197
    kMultiplyVectorByMatrix = 1198
    kMute = 932
    kNBase = 999
    kNCloth = 1008
    kNComponent = 995
    ...
    kPolyPyramid = 970
    kPolyQuad = 427
    kPolyReFormManip = 1155
    kPolyReFormManipContainer = 1154
    kPolyReduce = 771
    kPolyRemesh = 1114
    kPolySelectEditFeedbackManip = 1043
    kPolySeparate = 463
    kPolySewEdge = 698
    kPolySmartBevel = 1208
    kPolySmartExtrude = 1152
    kPolySmartExtrudeManip = 1153
    kPolySmooth = 428
...
class MObject(object):
    ...
    def apiTypeStr(self):
        if self._is_world:
            ...
            return _API_TYPES_INT_TO_FN_TYPE_STR.get(self._api_type[-1])
    ...
    def findPlug(self, attr_name: str, want_network_plug) -> 'MPlug':
        ...
        if properties is None:
            properties = attribute_properties.ATTRIBUTES_PROPERTIES.get(
                _get_node_type(attribute._owner), {}
            ).get(attribute._long_name, {})
...
class MFnTransform(MFnDagNode):
    ...
    def create(self, parent=None):
        ...
        """
        return super().create(MTypeId(_TYPE_STR_TO_ID["kTransform"]), parent=parent)
...
class MFnMesh(MFnDagNode):
    ...
    def create(self, *args, **kwargs):
        ...
        return super().create(MTypeId(_TYPE_STR_TO_ID["kMesh"]), name=mesh_name, parent=parent)
